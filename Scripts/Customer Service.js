...
if(!ASE_ID)
{
  window.location.assign("https://dct_tester.kh.asegroup.com:4751/CustomerService/login");
  //window.location.assign("http://localhost:4750/CustomerService/login");
}
    ...
    success: function (response) {
      ...
      if (response._source.reply[0] == null) {
        ...
        if (response._source.files[0] == null) {
          ...
          const fileName = response._source.files[0].name.toLowerCase();
          if (fileName.endsWith('jpg') || fileName.endsWith('png')) {
            ...
            msgHTML += '<p> A : <a href="' + response._source.files[0].base64 + '" download="' + response._source.files[0].name + '" target="_blank">'+response._source.files[0].name+'</a></p>';
          }
        }
      }
      else {
